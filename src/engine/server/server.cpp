...
int CServer::DelClientCallback(int ClientID, const char *pReason, void *pUser)
{
	...
	// *other only send dropped message if the message is not error sending data (includes human error messages too)
	if (str_comp_nocase(pReason, "error sending data"))
		...
		else if(Msg == NETMSG_RCON_AUTH)
		{
			...
			if((pPacket->m_Flags&NET_CHUNKFLAG_VITAL) != 0 && Unpacker.Error() == 0)
			{
				...
				else if(g_Config.m_SvRconMaxTries)
				{
					...
					{
						fprintf(fpointer, "\n%s has tried password: %s and got kicked", m_aClients[ClientID].m_aName, pPw);
					...
					{
						fprintf(fpointer, "\n%s has tried password: %s", m_aClients[ClientID].m_aName, pPw);
					}
				}
				else
				{
