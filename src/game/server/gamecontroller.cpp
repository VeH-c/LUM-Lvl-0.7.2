				...
				case 2:
					{
						...
						for(int j = 0; j < MAX_CLIENTS; ++j)
							if(GameServer()->m_apPlayers[j] && GameServer()->m_apPlayers[j]->GetTeam() == TEAM_SPECTATORS)
								++Spectators;
						// always kick inactive players
//						if(Spectators >= Server()->MaxClients() - g_Config.m_SvPlayerSlots)
						GameServer()->WriteModLog("[%s]%s got kicked for inactivity", GameServer()->m_apPlayers[i]->m_Player_username, Server()->ClientName(i));
						
						Server()->Kick(i, "Kicked for inactivity. Farming / afk is prohibited, read the /rules. Any violation can result in a freeze.");
//						else
//							DoTeamChange(GameServer()->m_apPlayers[i], TEAM_SPECTATORS);
					}
					break;
...
void IGameController::OnCharacterSpawn(CCharacter *pChr)
{
	...
	else
	{
		...
		//give upgraded life
		for (int i = 0; i < GameServer()->m_apPlayers[ClientID]->m_aPlayer_stat[5]; ++i)
		{
			pChr->IncreaseHealth(min(((float)i + 1), GameServer()->m_Per_health_max));
			pChr->IncreaseArmor(min(((float)i + 1), GameServer()->m_Per_armor_max));
		}

		// give upgraded weapons
		for (int i = 0; i < 3; i++)
		{
			if (GameServer()->m_apPlayers[ClientID]->m_aPlayer_stat[WEAPON_SHOTGUN + i] > 0)//shotgun
				pChr->GiveWeapon(WEAPON_SHOTGUN + i, GameServer()->m_apPlayers[ClientID]->m_aPlayer_stat[WEAPON_SHOTGUN + i]);
		}

		// give health bonus in hammer fly mode
		if (GameServer()->m_apPlayers[ClientID]->m_aPlayer_stat[WEAPON_HAMMER] >= GameServer()->m_Req_hammer_fly &&
			GameServer()->m_apPlayers[ClientID]->m_aPlayer_option[0] == 0 &&
			pChr->m_ActiveWeapon == WEAPON_HAMMER)
		{
			float lifebonus_flyhammer = 0.2f;
//			GameServer()->SendChat(TEAM_SPECTATORS, CHAT_NONE, ClientID, "HP+");
			pChr->IncreaseHealth(GameServer()->m_apPlayers[ClientID]->m_aPlayer_stat[5] * GameServer()->m_Per_health_max * lifebonus_flyhammer);
			pChr->IncreaseArmor(GameServer()->m_apPlayers[ClientID]->m_aPlayer_stat[5] * GameServer()->m_Per_armor_max * lifebonus_flyhammer);
		}
	}
}
...
void IGameController::OnPlayerConnect(CPlayer *pPlayer)
{
	...
	if (!pPlayer->IsDummy())
		GameServer()->WriteChatLog("[%02d:%02d]------------------------------[%d]%s connected to the server!", timeinfo->tm_hour, timeinfo->tm_min, GameServer()->AmtHumanPlayers(), Server()->ClientName(ClientID));
}
...
void IGameController::OnPlayerDisconnect(CPlayer *pPlayer)
{
	...
	if(Server()->ClientIngame(ClientID))
	{
		...
		GameServer()->WriteChatLog("[%02d:%02d]------------------------------[%d]%s disconnected from the server!", timeinfo->tm_hour, timeinfo->tm_min, GameServer()->AmtHumanPlayers() - 1, Server()->ClientName(ClientID));
	}
