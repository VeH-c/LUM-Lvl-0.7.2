...
CPlayer::CPlayer(CGameContext *pGameServer, int ClientID, bool Dummy)
{
	...
	// dummies need not login
	if (Dummy)
		m_Team = TEAM_RED;

	// if preferred secondary weapon is equal to primary weapon change that
	if (m_dum_wsecpref == m_dum_wprimpref)
		m_dum_wsecpref = m_dum_wprimpref == WEAPON_GUN ? WEAPON_HAMMER : WEAPON_GUN;
}
...
void CPlayer::Tick()
{
	...
	// logout delay
	if (m_LogOutDelay == 1)
		GameServer()->AccountLogOut(m_ClientID);

	if (m_LogOutDelay > 0)
		m_LogOutDelay--;

	// spawn protection countdown
	if (m_SpawnProtectionMs)
		m_SpawnProtectionMs--;

	// *dummy respawn quicker
...
void CPlayer::TryRespawn()
{
	...
	// spawn protection
	m_SpawnProtectionMs = GameServer()->m_SpawnProtectionBase * Server()->TickSpeed();
}
